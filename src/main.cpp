...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    //Always gives use the smallest number
    t = (-B - sqrt_disc) / 2*A;
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Matrix3d Matrix_A;
    Matrix_A << -pgram_u, -pgram_v, ray_direction;

    Vector3d t = Matrix_A.colPivHouseholderQr().solve(d);
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        if(is_light_visible(p+0.0001*Li, Li, light_position) == false){
            continue;
        }
        ...
        const Vector4d specular = obj_specular_color * std::pow(fmax(0, (N.dot(h))), obj_specular_exponent);
    ...
    Vector3d r =(d- (2 * N* (d.dot(N)))).normalized();

    Vector4d reflection_color(0, 0, 0, 0);
    reflection_color = refl_color.cwiseProduct(shoot_ray(p+(0.0001*r), r, max_bounce));
